<div class="product-<?php echo $cat_display == 'list' ? 'list' : 'grid'; ?>">
	<?php
	$c_prod = count($found_products);
	$i = 1;
	?>
	<?php if ($c_prod > 0) { ?>
		<?php foreach ($found_products AS $product) { ?>
    <?php //echo "<pre>";var_dump($product); exit; ?>
			<?php $img_url = prodImg($product->id, $product->image); ?>
			<div class="box-product <?php echo $c_prod == $i ? 'last-item' : ''; ?> <?php echo ($i++) % 3 == 1 ? ' row-first' : ''; ?> image-product-<?php echo $product->id; ?>"> 
                <a class="image" href="<?php echo product_url($product->id, $product->name); ?>" title="<?php echo $product->name; ?>"> <img src="<?php echo thumbImg($img_url, 300, 300); ?>" alt=""> </a>
				<div class="list_grid_right">

					<h3 class="name"><a href="<?php echo product_url($product->id, $product->name); ?>" title=""><?php echo $product->name; ?></a></h3>
					...
					<p class="submit">
						<input type="button" value="<?php echo $this->lang->line('Add to Cart'); ?>" class="button"  name="add-to-cart" prod-id="<?php echo $product->id; ?>">
					</p>
