<div class="box-content">
    <table class="table table-striped table-bordered bootstrap-datatable responsive">
        <?php if (count($blognews) > 0): ?>
            ...
            <tbody>
                <?php if (is_array($blognews) && count($blognews) > 0) {
                     foreach ($blognews as $item) { ?>
							<tr id="tr_<?php echo $item->id; ?>">
								<td class="image-in-list">
									...
									<a href="<?php echo blognews_url($item->id, $item->title); ?>"><img src="<?php echo $img_url; ?>"/><a/>
								</td>
								<td>
									<?php echo $item->title; ?>
								</td>
								...
								<td class="center red">
                                    <?php echo $blognews_comments_counts[$item->id]['pending_comments']; ?>
								</td>
								<td class="center">
                                    <?php echo $blognews_comments_counts[$item->id]['all_comments']; ?>
								</td>
								<td class="center">
									<a class="btn btn-info" href="<?php echo base_url("blognews/comments/" . $item->id); ?>" >
