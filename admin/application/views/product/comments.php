...
//    exit;
    ...
    <div >
        <table>
            <tr>
                <?php $img_url = prodImg($product['id'], $product['image']); ?> 
                <td><img src="<?php echo thumbImg($img_url, 200, 200, 100); ?>"/></td>
                <td>
                    <p><a href="<?php echo product_url($product['id'], $product['name_' . $default_language]); ?>" target="_blank"><?php echo $product['name_' . $default_language]; ?></a></p>
                    <p><?php echo $this->lang->line('Code') . " : " . $product['code']; ?></p>
                    <p><?php echo $this->lang->line('Price') . " : " . $product['price']; ?></p>
                </td>
            </tr>
        </table>
    </div>
                            ...
                            <?php foreach ($product_comments['comments'] AS $comment) { ?>
                                ...
                                <tr>
                                    ...
                                    <td class="center" style="vertical-align: middle;">
                                        ...
                                        <?php } else { ?>
                                            <span class="label-default label label-warning "  action="enable" comment_category="product" user_id="<?php echo $comment->user_id; ?>" product_id="<?php echo $product['id']; ?>" id="<?php echo $comment->id; ?>" status="change" title="<?php echo $this->lang->line('Press to activate'); ?>" style="cursor: pointer;"><?php echo $this->lang->line('Pending'); ?></span>
                                        <?php } ?>
                                    </td>
                                </tr>
                            <?php } ?>
...
<script  type="text/javascript">
    ...
    $(document).ready(function() {
        $(document).on('click', 'span[status="change"]', function() {
            ...
            var comment_category = $(this).attr('comment_category');
            var table = 'product_comments';
            var id = $(this).attr('id');
            $.ajax({
                url: base_url + "ajax",
                dataType: 'json',
            ...
            $(document).on('blur', 'textarea[name=edit_comment_text_area]', function() {
                ...
                var $_this = $(this);
                var table_name = 'product_comments';
                var text = $(this).val();
                var comment_id = $(this).attr('comment-id');
