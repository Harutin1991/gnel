...
class Partner extends Main_controller {
    ...
    public function index() {
        $this->data['partners'] = $this->PartnerModel->getAll('partners');
        $this->load->view('partner/index', $this->data);
    }

    public function add() {
        if ($this->input->post('Partner')) {
            $this->form_validation->set_rules($this->PartnerModel->rules());
            if ($this->form_validation->run()) {
                $data = $this->input->post('Partner', true);

                $image_data = $this->_do_upload('image', 'partner');
                if (isset($image_data['upload_data']))
                    $data['image'] = $image_data['upload_data']['file_name'];
                $data = trim_array($data);
                if (($id = $this->PartnerModel->insert('partners', $data)) != false) {
                    $this->addLog('Added page with id: ' . $id);
                }

                $this->session->set_flashdata('message', 'add_success');
                redirect("partner/edit/$id", 'refresh');
            }
        }

        $this->load->view('partner/add', $this->data);
    }

    public function edit($id = NULL) {
        if (!isset($id))
            show_404();
        $this->data['partner'] = $this->PartnerModel->get('partners', $id, $this->session->userdata('admin_id'));
        if ($this->data['partner'] == false)
            show_404();
        ...
        if ($this->input->post('Partner')) {
            ...
            if ($this->form_validation->run()) {
                $data = $this->input->post('Partner', true);
                $old_image = $this->data['partner']['image'];

                if (!empty($_FILES['image']['name'])) {
                    $image_data = $this->_do_upload('image', 'partner');

                    if (isset($old_image))
                        $this->_delete_img($old_image, 'partner');

                    if (isset($image_data['upload_data']))
                        $data['image'] = $image_data['upload_data']['file_name'];
                }
                $data = trim_array($data);
                if ($this->PartnerModel->update('partners', $id, $data) != false) {
        ...
        $this->load->view('partner/edit', $this->data);
    }
